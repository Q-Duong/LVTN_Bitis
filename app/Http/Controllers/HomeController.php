...
class HomeController extends Controller
{
    ...
    public function indexpage(){
        ...
        if(Session::get('welcome')){
            ...
            $all_post = Post::where('post_status','1')->inRandomOrder('post_id')->limit(4)->get();
            
            $all_product_new =  Product::where('product_active','1')->where('product_status','2')->where('product_quantity','>=',1)->inRandomOrder('product_id')->limit(8)->get(); 

            $all_product_sale =  Product::where('product_active','1')->where('product_status','1')->where('product_quantity','>=',1)->inRandomOrder('product_id')->limit(8)->get(); 

            return view('pages.home')->with('category',$cate_product)->with('all_product_new',$all_product_new)->with('category_post',$category_post)->with('all_product_sale',$all_product_sale)->with('slider',$slider)->with('all_post',$all_post);
        }else{
    ...
    public function index(){
        ...
        $slider = Slider::orderBy('slider_id','ASC')->where('slider_status','1')->get();
    ...
    public function product(){
        ...
        $all_product =  Product::where('product_active','1')->inRandomOrder('product_id')->Paginate(27); 
    ...
    public function blog_list(){
        ...
        $all_product = Product::where('product_active','1')->inRandomOrder('product_id')->get(); 
    ...
    public function autocomplete_ajax(Request $request){
        ...
        if($data['query']){
            ...
            $product = Product::where('product_active',1)->where('product_name','LIKE','%'.$data['query'].'%')->inRandomOrder('product_id')->limit(10)->get();
    ...
    public function wistlist(Request $request){
       ...
       $all_product =  Product::where('product_active','1')->where('product_quantity','>=',1)->inRandomOrder('product_id')->get(); 
